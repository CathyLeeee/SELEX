...
def make_seq_RNAME_dict(path_of_resorted_fq1 , path_of_resorted_fq2, seq_strandness_dict):
    ...
    """
    global l_temp_RNAME_for_each_seq,seq_RNAME_dict
    ...
    seq_RNAME_dict={}
    for seq in seq_strandness_dict :
        l_temp_RNAME_for_each_seq=[]
        l_index=[i for i, x in enumerate(l_fq) if x==seq]
        for index in l_index:
            l_temp_RNAME_for_each_seq.append(l_fq[index-1].split()[0].strip("@"))
        l_temp_RNAME_for_each_seq=list(set(l_temp_RNAME_for_each_seq))
        seq_RNAME_dict[seq]=l_temp_RNAME_for_each_seq
